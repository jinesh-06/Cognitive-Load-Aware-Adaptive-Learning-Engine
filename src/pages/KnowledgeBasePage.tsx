...
import { fetchDocuments, searchRAG, uploadDocument } from '../services/api';

export const KnowledgeBasePage: React.FC = () => {
  const [chunks, setChunks] = useState<RAGDocumentChunk[]>([]);
  const [stats, setStats] = useState<any>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState<RAGDocumentChunk[]>([]);
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12" id="knowledge-base-container">
      {/* Top Banner & Stats */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow transition-all flex items-center gap-2"
          >
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-slate-200 space-y-6">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-2"
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in" id="doc-upload-backdrop">
            ...
            <form onSubmit={handleUploadSubmit} className="p-6 space-y-4 text-xs">
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold shadow"
                >
