...
import { AdaptedContent, BehavioralFeatures, CognitiveLoadState, LessonTopic, PredictionResult, RAGDocumentChunk } from '../types';
import { generateAdaptedContent, submitQuiz } from '../services/api';
...
export const LearningPage: React.FC<Props> = ({
  topics,
  currentTopic,
  onSelectTopic,
  currentLoad,
  ...
  const [viewMode, setViewMode] = useState<'adapted' | 'original'>('adapted');
  const [adaptedContent, setAdaptedContent] = useState<AdaptedContent | null>(null);
  const [ragSources, setRagSources] = useState<RAGDocumentChunk[]>([]);
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="learning-page-container">
            ...
            <div className="space-y-3">
                ...
                {activeQuestion.options.map((opt, optIdx) => {
                  ...
                  return (
                    <button
                      ...
                      id={`quiz-opt-${optIdx}`}
                      className={`w-full p-3 rounded-xl border text-xs text-left transition-all flex items-start gap-2.5 ${optStyle}`}
                    >
              ...
              {!quizSubmitted ? (
                <button
                  ...
                  id="submit-quiz-btn"
                  className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                ...
                <div className="space-y-3 animate-fade-in pt-2">
                  ...
                  <div className="flex flex-col gap-2 pt-1">
                    <div className="flex items-center gap-2">
                      ...
                      {currentQuizIndex < currentTopic.quiz.length - 1 ? (
                        <button
                          ...
                          className="flex-1 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5"
                        >
                          Next Question
                        ...
                        <button
                          ...
                          className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow"
                    ...
                    {feedbackSuccessMessage && (
                      <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-xl text-indigo-950 text-xs space-y-2 animate-fade-in mt-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-indigo-100">
                          ...
                          {topics.findIndex(t => t.id === currentTopic.id) < topics.length - 1 && (
                            <button
                              ...
                              className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[11px] font-semibold flex items-center gap-1 transition-colors"
                            >
                              Next Topic
