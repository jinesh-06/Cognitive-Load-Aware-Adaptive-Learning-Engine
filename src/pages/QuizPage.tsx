...
import { BehavioralFeatures, CognitiveLoadState, LessonTopic, PredictionResult, QuizQuestionAttemptResult } from '../types';
import { saveQuizAttempt, submitQuiz } from '../services/api';
...
interface Props {
  ...
  onUpdateFeatures?: (features: Partial<BehavioralFeatures>) => void;
  onTriggerRecalculate: (overrideFeatures?: Partial<BehavioralFeatures>) => Promise<PredictionResult | void> | void;
  onTriggerReRead: () => void;
  onNavigateToReading: () => void;
  onNavigateToHistory?: () => void;
  onForceLoad: (load: CognitiveLoadState) => void;
}
...
export const QuizPage: React.FC<Props> = ({
  ...
  const [quizAnswers, setQuizAnswers] = useState<Record<number, QuestionFeedbackState>>({});
  ...
  const handleQuizSubmit = async () => {
    ...
    try {
      ...
      const record: QuestionFeedbackState = {
      ...
      if (Object.keys(updatedAnswers).length === currentTopic.quiz.length) {
        const fullQuestionsData: QuizQuestionAttemptResult[] = currentTopic.quiz.map((q, idx) => {
  ...
  const handleSendFeedbackToModel = async () => {
    ...
    const payloadUpdates: Partial<BehavioralFeatures> = {
  ...
  const getLoadBadgeColor = (load: CognitiveLoadState) => {
    switch (load) {
      case 'LOW':
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="quiz-page-container">
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 shadow-xs border border-slate-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          ...
          <button
            ...
            className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 self-start sm:self-auto bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg border border-indigo-200 transition-colors"
          >
          ...
          {topics.map(topic => {
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-semibold shrink-0 transition-all text-left flex flex-col gap-0.5 border ${
                  isSelected
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-slate-200 space-y-6" id="checkpoint-quiz-card">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              ...
              <button
                ...
                className="px-3.5 py-2 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold flex items-center gap-2 transition-all shadow-2xs hover:scale-[1.02] active:scale-95 self-start sm:self-auto"
            ...
            {reReadNotice && (
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-900 flex items-center gap-2 animate-fade-in">
              ...
              {!quizSubmitted ? (
                <div className="flex flex-col sm:flex-row items-center gap-3 pt-3">
                  <button
                    ...
                    id="submit-quiz-btn"
                    className="w-full sm:flex-1 py-3.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs sm:text-sm font-bold rounded-xl shadow transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    onClick={handleReReadAction}
                    className="w-full sm:w-auto px-4 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5"
                  ...
                  <div className="flex flex-col gap-2.5 pt-1">
                    <div className="flex flex-wrap items-center gap-2">
                      {currentQuizIndex > 0 && (
                        <button
                          ...
                          className="py-2.5 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5"
                        >
                      ...
                      {currentQuizIndex < currentTopic.quiz.length - 1 ? (
                        <button
                          ...
                          className="flex-1 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5"
                        >
                          Next Question
                        ...
                        <>
                          <button
                            ...
                            className="py-2.5 px-4 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
                          ...
                          <button
                            ...
                            className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow"
                    ...
                    {currentQuizIndex < currentTopic.quiz.length - 1 && (
                      <button
                        onClick={handleSendFeedbackToModel}
                        disabled={isRecalibrating}
                        className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow"
                      >
                        <RefreshCw className={`w-3.5 h-3.5 ${isRecalibrating ? 'animate-spin' : ''}`} />
                    ...
                    {feedbackSuccessMessage && (
                      <div className="p-3.5 bg-indigo-50 border border-indigo-200 rounded-xl text-indigo-950 text-xs space-y-2 animate-fade-in mt-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-indigo-100">
                          <button
                            onClick={handleTryAgain}
                            className="px-2.5 py-1 bg-amber-50 hover:bg-amber-100 border border-amber-300 text-amber-900 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-colors"
                          >
                          ...
                          <button
                            onClick={onNavigateToReading}
                            className="px-2.5 py-1 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-lg text-[11px] font-semibold flex items-center gap-1 transition-colors"
                          >
                          ...
                          {isAllCorrect && topics.findIndex(t => t.id === currentTopic.id) < topics.length - 1 && (
                            <button
                              ...
                              className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[11px] font-semibold flex items-center gap-1 transition-colors shadow-2xs"
                            >
                              Next Topic
      ...
      {isAllCorrect ? (
        (() => {
          ...
          return (
            <div
              className="bg-gradient-to-r from-emerald-950 via-slate-900 to-indigo-950 text-white rounded-3xl p-6 sm:p-8 shadow-lg border border-emerald-800/40 flex flex-col sm:flex-row items-center justify-between gap-6 animate-fade-in"
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-3 shrink-0">
                <button
                  onClick={handleTryAgain}
                  className="px-4 py-3 bg-slate-800/80 hover:bg-slate-700 text-slate-200 font-semibold text-xs sm:text-sm rounded-2xl border border-slate-700 transition-all flex items-center gap-2"
                ...
                {onNavigateToHistory && (
                  <button
                    ...
                    className="px-4 py-3 bg-slate-800/80 hover:bg-slate-700 text-indigo-300 font-semibold text-xs sm:text-sm rounded-2xl border border-slate-700 transition-all flex items-center gap-2"
                ...
                {hasNextTopic && (
                  <button
                    ...
                    id="btn-next-topic"
                    className="px-6 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-xs sm:text-sm rounded-2xl shadow-md shadow-emerald-500/30 flex items-center gap-2 transition-all transform hover:-translate-y-0.5 active:scale-95"
                  >
        ...
        <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-600">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {answeredEntries.length > 0 && (
              <button
                onClick={handleTryAgain}
                className="px-3.5 py-2 bg-amber-50 hover:bg-amber-100 text-amber-900 font-bold rounded-xl border border-amber-300 transition-colors flex items-center gap-1.5 shrink-0"
              >
                <RefreshCw className="w-3.5 h-3.5 text-amber-700" />
            ...
            {onNavigateToHistory && (
              <button
                onClick={onNavigateToHistory}
                className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 font-semibold rounded-xl border border-indigo-200 transition-colors flex items-center gap-1.5 shrink-0"
              >
            ...
            <button
              onClick={onNavigateToReading}
              className="px-3.5 py-2 bg-white hover:bg-slate-100 text-slate-700 font-semibold rounded-xl border border-slate-200 transition-colors flex items-center gap-1.5 shrink-0"
            >
              <BookOpen className="w-3.5 h-3.5 text-slate-500" />
