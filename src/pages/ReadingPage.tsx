...
import { AdaptedContent, BehavioralFeatures, CognitiveLoadState, LessonTopic, RAGDocumentChunk } from '../types';
import { generateAdaptedContent } from '../services/api';
...
export const ReadingPage: React.FC<Props> = ({
  topics,
  currentTopic,
  onSelectTopic,
  currentLoad,
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="reading-page-container">
      ...
      {features.number_of_re_reads > 0 && (
        <div className="bg-amber-50 border border-amber-200/90 rounded-2xl p-3.5 px-4 text-amber-900 text-xs flex items-center justify-between gap-3 shadow-xs">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-white rounded-lg font-semibold text-xs transition-colors shrink-0 flex items-center gap-1"
          >
            Return to Quiz
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-indigo-900 via-indigo-950 to-slate-900 text-white rounded-3xl p-6 sm:p-8 shadow-lg border border-indigo-800/50 flex flex-col sm:flex-row items-center justify-between gap-6" id="proceed-to-quiz-cta">
          ...
          <button
            ...
            className="px-6 py-3.5 bg-indigo-500 hover:bg-indigo-400 text-white font-bold text-sm rounded-2xl shadow-md shadow-indigo-500/30 flex items-center gap-2 transition-all transform hover:-translate-y-0.5 active:scale-95 shrink-0"
          >
